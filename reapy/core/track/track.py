...
from reapy.core.project.project import Project
from reapy.core.audio_accessor import AudioAccessor
from reapy.core.fx.fx import FX
from reapy.core.item.item import Item
from reapy.errors import InsideREAPERError, InvalidObjectError, UndefinedEnvelopeError
import typing as ty
...
class Track(ReapyObject):
    ...
    def add_fx(
        ...
        fx = reapy.FX(ty.cast(reapy.Track, self), index)
        return fx
    ...
    def add_item(
        ...
        item = reapy.Item(RPR.AddMediaItemToTrack(self.id))  # type:ignore
        item.position = start
    ...
    def add_send(self, destination: ty.Optional['Track'] = None):
        ...
        send = reapy.Send(ty.cast(reapy.Track, self), send_id, type=type)
        return send
    ...
    def envelopes(self) -> 'reapy.EnvelopeList':
        ...
        :type: EnvelopeList
        """
        return reapy.EnvelopeList(ty.cast(reapy.Track, self))
    ...
    @classmethod
    def _from_GUID_all_projects(cls, guid_string: str) -> 'reapy.Track':
    ...
    def fxs(self) -> 'reapy.FXList':
        ...
        :type: FXList
        """
        fxs = reapy.FXList(ty.cast(reapy.Track, self))
        return fxs
    ...
    @reapy.inside_reaper()
    def _items_inside(self) -> ty.List['reapy.Item']:
    ...
    def parent_track(self) -> ty.Optional['reapy.Track']:
        ...
        """
        parent = reapy.Track(RPR.GetParentTrack(self.id))  # type:ignore
    ...
    @reapy.inside_reaper()
    def _recieves_inside(self) -> ty.List['reapy.Send']:
        return [
            reapy.Send(ty.cast(reapy.Track, self), i, type="receive")
    ...
    @reapy.inside_reaper()
    def _send_inside(self) -> ty.List['reapy.Send']:
        return [
            reapy.Send(reapy.Track(self.id), i, type="send")
...
class TrackList(ReapyObjectList):
    ...
    @ty.overload
    def __getitem__(self, key: slice) -> ty.List['reapy.Track']:
        ...

    @ty.overload
    def __getitem__(self, key: int) -> 'reapy.Track':
        ...

    def __getitem__(self, key):
        if isinstance(key, slice):
            return self._get_items_from_slice(key)
        return reapy.Track(key, self.parent)
    ...
    @reapy.inside_reaper()
    def _get_items_from_slice(self, slice: slice) -> ty.List['reapy.Track']:
